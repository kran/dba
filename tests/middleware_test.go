...
func TestMiddleware_Logging(t *testing.T) {
	...
	logged := q.Use(func(next stupidql.ExecFunc) stupidql.ExecFunc {
...
func TestMiddleware_ShortCircuit(t *testing.T) {
	...
	blocked := q.Use(func(next stupidql.ExecFunc) stupidql.ExecFunc {
		return func(ctx context.Context, query string, args []any) (any, error) {
...
func TestMiddleware_Chain_Order(t *testing.T) {
	...
	chained := q.
		Use(func(next stupidql.ExecFunc) stupidql.ExecFunc {
			return func(ctx context.Context, query string, args []any) (any, error) {
				record("A-before")
				result, err := next(ctx, query, args)
				record("A-after")
...
func TestMiddleware_Immutable(t *testing.T) {
	...
	called := false
	withMW := q.Use(func(next stupidql.ExecFunc) stupidql.ExecFunc {
		return func(ctx context.Context, query string, args []any) (any, error) {
			called = true
...
func TestLogMiddleware_SlowQuery(t *testing.T) {
	...
	// 1ns 阈值，任何查询都算慢
	logged := q.Use(stupidql.LogMiddleware(logger, 1*time.Nanosecond))
